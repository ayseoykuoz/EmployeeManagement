...
describe('EmployeeList', () => {
  before(async () => {
    await loadLanguage('en'); // Ensure translations are loaded
  });
  ...
  it('updates when the language changes', async () => {
    ...
    expect(titleElement.textContent.trim()).to.equal('Çalışan Listesi'); // Assuming this is the translation
  });
  ...
  it('updates localization when language changes', async () => {
    ...
    // Dispatch the language-changed event with the language detail
